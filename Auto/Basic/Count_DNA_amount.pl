#!/usr/bin/perl
# 2017/10/17 6500bp以下だけに限定する
# 2016/10/25 self ligationだとmboIのデータには存在しないことがあるのでself + undigestにする
# 2016/10/24 各fragmentがどれだけデータに存在するのかを計算するように修正
# 2016/09/14 self ligationだけに絞って数を数える
...
foreach my $chr(@Chromosomes){
	my $sth_data = $dbh->prepare("select position1, position2, direction1, direction2 from map
			where chr1=chr2 and abs(position1 - position2) < 600 and chr1='$chr'");
	$dbh->do('BEGIN');
	$sth_data->execute();

	# 1kbごとにオーバーラップしている数を数える
	#　異なる向きの組み合わせ- 同じ向きの組み合わせ
	my %Data;
