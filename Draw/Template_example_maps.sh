...
Format of location file should have following columns, resolution, color, chr, start, end
File should be deliminated by tab. Order is flexible but should have header as described in above.
...
do
	...
	COL=$(sqlite3 ${DB_loc} "select color from loc where id='${id}'")
...
sbatch -n 4 --job-name=1 $(sq --node) -o "${DIR_OUT}/log/${TIME_STAMP}_report.log" --open-mode append <<-EOFFF
#!/bin/sh
