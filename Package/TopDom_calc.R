...
option_list <- list(  
  ...
  make_option(c("-o", "--out"), help="output domain bed file"),
  make_option(c("-w", "--window"), default=5, help="window bin size"),
  make_option(c("-f", "--filter"), default=TRUE, help="filtering folse positive boundaries")
)
...
Which.Gap.Region2 <- function(matrix.data, w)
{
  ...
  # print(matrix.data[1:5,1:5])
  
  for(i in 1:n_bins)
  {
    if( sum( matrix.data[i, max(1, i-w):min(i+w, n_bins)] ) == 0 ){
      gap[i]=-0.5
    }
  }
...
statFilter <- eval(parse(text=as.character(opt["filter"])))
# TopDom <- function( matrix.file, window.size, outFile=NULL, statFilter=T)
{
  ...
  window.size = as.numeric(opt["window"])
  ...
  LocMatrix[,3] <- as.numeric(LocMatrix[,3])+1
  colnames(LocMatrix) <- c("chr", "from.coord", "to.coord")
  ...
  bins <- data.frame(id=1:n_bins, 
                     chr=as.character(LocMatrix[,1]), 
                     from.coord=as.numeric(LocMatrix[,2]), 
                     to.coord=as.numeric(LocMatrix[,3]))
  matrix.data <- map
  ...
  OUT_domain <- bedform[bedform[,"name"] == "domain",c("chrom", "chromStart", "chromEnd")]
  
  options(scipen=10)
  write.table(OUT_domain, file=as.character(opt["out"]), quote = FALSE, row.names = FALSE, col.names = FALSE, sep="\t", eol = "\n")
  # if( !is.null(outFile) ) {
  #   print("#########################################################################")
  #   print("Writing Files")
  #   print("#########################################################################")
